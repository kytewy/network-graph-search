...
export function NodeTooltip({
  ...
  // Clear any hide timeout when component unmounts
  useEffect(() => {
    return () => {
  ...
  useEffect(() => {
    if (isHovering && hideTooltipTimeoutRef.current) {
      clearTimeout(hideTooltipTimeoutRef.current)
  ...
  return (
    <Card
      ...
      onMouseLeave={() => {
        setIsHovering(false)
        // Add delay before closing the tooltip
      ...
      onWheel={(e) => {
        // Prevent the tooltip from disappearing while scrolling
        e.stopPropagation()
        if (hideTooltipTimeoutRef.current) {
          clearTimeout(hideTooltipTimeoutRef.current)
          hideTooltipTimeoutRef.current = null
        ...
        // Allow scrolling within the tooltip content
        const summaryElement = tooltipRef.current?.querySelector('.overflow-y-auto')
        if (summaryElement) {
          summaryElement.scrollTop += e.deltaY
        }
      }}
      onClick={(e) => {
        // Prevent clicks from propagating to canvas
        e.stopPropagation()
        // Keep tooltip visible when clicked
        setIsHovering(true)
        if (hideTooltipTimeoutRef.current) {
          clearTimeout(hideTooltipTimeoutRef.current)
          hideTooltipTimeoutRef.current = null
...
export function NodeModal({
  ...
  // Prevent clicks from closing the modal when clicking inside it
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      if (modalRef.current && e.target instanceof Element && !modalRef.current.contains(e.target)) {
        onClose()
      }
    }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div 
        ref={modalRef}
        className="relative bg-white border border-gray-200 shadow-xl rounded-lg max-w-2xl max-h-[90vh] w-full mx-4 overflow-hidden flex flex-col"
      >
