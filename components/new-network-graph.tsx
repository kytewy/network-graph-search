...
export default function NetworkGraph({
  ...
  const { selections, onNodeClick: handleNodeClick, onCanvasClick } = useSelection({
    ref,
    nodes: graphNodes,
    edges: graphEdges,
  })
  ...
  const handleCustomNodeClick = (node: GraphNode) => {
    // Call the built-in handler for selection
    ...
    <div className="network-graph-container relative w-full h-full" onMouseMove={handleMouseMove}>
      <GraphCanvas
        ...
        edgeWidth={(edge) => {
          ...
          return isHighlighted || isNodeHighlighted ? 2 : Math.max(1, (edge.data as Link).strength)
        }}
        getNodePosition={(id) => {
                ...
                {showTooltipDropdown && (
                  ...
                  >
                    ...
                    {onNodeExpand && (
                      ...
                      >
                        {expandedNodes?.includes(hoveredNode.id) ? "Collapse Node" : "Expand Node"}
                      </button>
                    )}
                    ...
                    {showModalDropdown && (
                      <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-md py-1 min-w-[160px] z-10 overflow-hidden shadow-lg">
                        ...
                        {onNodeExpand && (
                          ...
                          >
                            {expandedNodes?.includes(selectedArticle.id) ? "Collapse Node" : "Expand Node"}
                          </button>
                        )}
                        <div className="border-t border-gray-200 my-1"></div>
                        <button
              ...
              <div className="space-y-4">
                <div>
                  ...
                  >
                    ...
                    <svg
                      className={`w-5 h-5 text-[#6b7280] transition-transform duration-200 ${summaryExpanded ? "rotate-180" : ""}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                  ...
                  {summaryExpanded && (
                    <div className="mt-2 p-3 border border-gray-200 rounded-md bg-white">
                      <p className="text-sm text-[#374151] leading-relaxed">{selectedArticle.summary}</p>
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <svg
                      className={`w-5 h-5 text-[#6b7280] transition-transform duration-200 ${contentExpanded ? "rotate-180" : ""}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                  ...
                  {contentExpanded && (
                    <div className="mt-2 p-3 border border-gray-200 rounded-md bg-white max-h-[400px] overflow-y-auto">
                      <p className="text-sm text-[#374151] leading-relaxed whitespace-pre-wrap">{selectedArticle.content}</p>
                    </div>
                  )}
                </div>
              </div>
